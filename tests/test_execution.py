...
class ExecutionTests(DeploymentTest):
    ...
    def test_source_connection_limit_serializes_branches(self):
        ...
        result = self.run_dag()
        first, second = (result["nodes"][key] for key in ("extract_customers", "extract_orders"))
        self.assertLessEqual(first["ended_at"], second["started_at"])

    def test_conditional_execution_and_completion_cleanup(self):
        self.data["notifications"]["enabled"] = False
        result = self.run_dag()
        self.assertEqual(result["status"], "SUCCEEDED")
        self.assertEqual(result["nodes"]["deliver_report"]["status"], Status.SKIPPED_CONDITION)
    ...
    def test_retryable_failure(self):
        self.fault("transform_orders", "retryable")
        result = self.run_dag()
        self.assert_report(result)
        self.assertEqual(result["nodes"]["transform_orders"]["attempt"], 2)
    ...
    def test_retry_backoff_respects_delay_and_cap(self):
        self.fault("transform_orders", "retryable", attempts=[1, 2])
        self.node("transform_orders")["retry"].update(
            max_retries=2,
            delay_seconds=0.15,
            backoff=2,
        ...
        for event, expected in zip(retries, [0.15, 0.2], strict=True):
            ...
            started = next(
                ...
                and entry["node_id"] == "transform_orders"
    ...
    def test_non_retryable_failure_and_dependency_blocking(self):
        self.fault("transform_orders")
        result = self.run_dag()
        self.assertEqual(result["status"], "FAILED")
        self.assertEqual(result["nodes"]["transform_orders"]["attempt"], 1)
        self.assertEqual(result["nodes"]["quality_gate"]["status"], Status.BLOCKED)
        self.assertEqual(result["nodes"]["publish_report"]["status"], Status.BLOCKED)
    ...
    def test_recovery_reuses_proven_success(self):
        ...
        self.assertEqual(recovered["nodes"]["transform_orders"]["status"], Status.SUCCEEDED)
    ...
    def test_repeated_failure_during_recovery(self):
        ...
        self.assertEqual(third["nodes"]["extract_orders"]["reused_from_run_id"], first["run_id"])

    def test_force_restart_invalidates_only_selected_downstream(self):
        first = self.run_dag()
        result = self.run_dag("force_restart", first["run_id"], "extract_orders")
        self.assert_report(result)
        self.assertEqual(result["restart_mode"], "force_restart")
        self.assertEqual(result["restart_origin"], "extract_orders")
        self.assertEqual(result["nodes"]["extract_customers"]["status"], Status.SKIPPED_ALREADY_SATISFIED)
        for key in ("extract_orders", "transform_orders", "publish_report"):
    ...
    def test_force_restart_from_named_checkpoint(self):
        ...
        self.assertEqual(result["nodes"]["transform_orders"]["status"], Status.SKIPPED_ALREADY_SATISFIED)

    def test_force_rerun_successful_run_without_duplicate_side_effect(self):
        first = self.run_dag()
        before = self.assert_report(first)
        second = self.run_dag("force_rerun")
        self.assertEqual(self.assert_report(second), before)
    ...
    def test_relevant_configuration_change_invalidates_success(self):
        ...
        first = self.run_dag()
        self.data["runtime"]["fault_injection"].clear()
        self.data["configuration_version"] = "1.0.1"
        self.data["data_quality"]["minimum_accepted_rows"] = 2
        second = self.run_dag("resume", first["run_id"])
        self.assert_report(second)
        self.assertEqual(second["nodes"]["extract_orders"]["status"], Status.SKIPPED_ALREADY_SATISFIED)
        self.assertEqual(second["nodes"]["transform_orders"]["status"], Status.SUCCEEDED)
    ...
    def test_retry_policy_change_preserves_unrelated_success(self):
        ...
        first = self.run_dag()
        self.data["runtime"]["fault_injection"].clear()
        self.node("extract_orders")["retry"]["delay_seconds"] = 3
        second = self.run_dag("resume", first["run_id"])
        self.assert_report(second)
        self.assertEqual(second["nodes"]["extract_orders"]["status"], Status.SKIPPED_ALREADY_SATISFIED)
    ...
    def test_missing_output_invalidates_its_dependents(self):
        self.fault("publish_report")
        first = self.run_dag()
        Path(first["nodes"]["extract_orders"]["outputs"]["orders"]["path"]).unlink()
        self.data["runtime"]["fault_injection"].clear()
        second = self.run_dag("resume", first["run_id"])
        self.assert_report(second)
        self.assertEqual(second["nodes"]["extract_orders"]["status"], Status.SUCCEEDED)
        self.assertEqual(second["nodes"]["extract_customers"]["status"], Status.SKIPPED_ALREADY_SATISFIED)
        self.assertEqual(second["nodes"]["transform_orders"]["status"], Status.SUCCEEDED)

    def test_corrupt_output_cannot_be_reused(self):
        self.fault("quality_gate")
        first = self.run_dag()
        artifact = Path(first["nodes"]["transform_orders"]["outputs"]["accepted"]["path"])
    ...
    def test_incomplete_checkpoint_proof_is_not_reused(self):
        ...
        def damage_proof(tx):
            key = store.node_key(first["run_id"], "transform_orders")
    ...
    def test_non_utf8_output_is_invalidated(self):
        self.fault("quality_gate")
        first = self.run_dag()
        Path(first["nodes"]["transform_orders"]["outputs"]["accepted"]["path"]).write_bytes(b"\xff\xfe\xff")
        self.data["runtime"]["fault_injection"].clear()
        second = self.run_dag("resume", first["run_id"])
        self.assert_report(second)
    ...
    def test_source_content_change_prevents_stale_skip(self):
        self.fault("quality_gate")
        first = self.run_dag()
        source = self.root / "sample_data" / "orders.csv"
        source.write_text(source.read_text() + "o6,c1,100\n")
        self.data["runtime"]["fault_injection"].clear()
        second = self.run_dag("resume", first["run_id"])
        self.assertEqual(second["status"], "SUCCEEDED")
        self.assertEqual(second["nodes"]["extract_orders"]["status"], Status.SUCCEEDED)
        rows = self.last_engine.services.artifacts.read(
            second["nodes"]["publish_report"]["outputs"]["report"]
        )
        self.assertEqual(next(row for row in rows if row["region"] == "North")["total_cents"], 1600)
    ...
    def test_side_effect_commit_before_failure_is_deduplicated_on_retry(self):
        self.fault("deliver_report", "after_effect")
        result = self.run_dag()
        self.assert_report(result)
        self.assertEqual(result["nodes"]["deliver_report"]["attempt"], 2)
        events = self.last_engine.store.backend.events(self.scope())
    ...
    def test_changed_payload_cannot_reuse_same_side_effect_key(self):
        ...
        second = self.run_dag("force_rerun")
        self.assertEqual(second["status"], "FAILED")
        self.assertEqual(second["nodes"]["deliver_report"]["error"]["code"], "SIDE_EFFECT_CONFLICT")
        self.assertEqual(len(self.last_engine.store.view(self.scope()).all("outbox")), 1)
        self.assertNotEqual(first["run_id"], second["run_id"])

    def test_pending_delivery_reference_is_repaired_without_duplicate_intent(self):
        first = self.run_dag()
        Path(first["nodes"]["publish_report"]["outputs"]["report"]["path"]).unlink()
        second = self.run_dag("force_restart", first["run_id"], "publish_report")
        self.assert_report(second)
        outbox = self.last_engine.store.view(self.scope()).all("outbox")
        self.assertEqual(
            outbox[0]["output_reference"], second["nodes"]["publish_report"]["outputs"]["report"]
        )
        events = self.last_engine.store.backend.events(self.scope())
        self.assertEqual(sum(event["event_type"] == "OUTBOX_COMMITTED" for event in events), 1)
    ...
    def test_manual_non_idempotent_work_requires_approval(self):
        ...
        self.assertEqual(blocked["nodes"]["deliver_report"]["error"]["code"], "APPROVAL_REQUIRED")
        approved = self.run_dag("resume", blocked["run_id"], approvals=("deliver_report",))
        self.assert_report(approved)
    ...
    def test_compensation_requires_validated_companion_and_approval(self):
        ...
        self.data["runtime"]["fault_injection"].clear()
        recovered = self.run_dag("resume", first["run_id"], approvals=("deliver_report",))
        self.assert_report(recovered)
    ...
    def test_manual_approval_cannot_be_bypassed_by_an_intermediate_blocked_run(self):
        ...
        blocked = self.run_dag("force_restart", first["run_id"], "extract_orders")
        self.assertEqual(blocked["nodes"]["deliver_report"]["attempt"], 0)
        self.data["runtime"]["fault_injection"].clear()
        rejected = self.run_dag("resume", blocked["run_id"])
        self.assertEqual(rejected["nodes"]["deliver_report"]["error"]["code"], "APPROVAL_REQUIRED")
    ...
    def test_missing_source_fails_bounded_retries_without_abandoning_run(self):
        ...
        self.assertEqual(result["nodes"]["extract_orders"]["error"]["code"], "SOURCE_UNAVAILABLE")
        self.assertEqual(result["nodes"]["transform_orders"]["status"], Status.BLOCKED)
    ...
    def test_fail_fast_still_runs_failure_and_completion_handlers(self):
        ...
        result = self.run_dag()
        self.assertEqual(result["status"], "FAILED")
        self.assertEqual(result["nodes"]["extract_orders"]["status"], Status.CANCELLED)
        self.assertEqual(result["nodes"]["notify_failure"]["status"], Status.SUCCEEDED)
        self.assertEqual(result["nodes"]["cleanup"]["status"], Status.SUCCEEDED)
    ...
    def test_run_deadline_cancels_ready_work_without_busy_loop(self):
        ...
        self.data["concurrency"]["max_parallel"] = 1
        self.fault("extract_customers", "timeout", delay_seconds=0.2)
    ...
    def test_partial_artifact_write_is_not_published_as_success(self):
        self.node("transform_orders")["retry"]["max_retries"] = 0
        self.fault("transform_orders", "after_output")
        first = self.run_dag()
        self.assertEqual(first["nodes"]["transform_orders"]["outputs"], {})
        self.assertIsNone(first["nodes"]["transform_orders"]["checkpoint"])
        self.data["runtime"]["fault_injection"].clear()
        second = self.run_dag("resume", first["run_id"])
        self.assert_report(second)

    def test_audit_completeness(self):
        self.fault("extract_orders", "retryable")
        result = self.run_dag()
        events = self.last_engine.store.backend.events(self.scope())
    ...
    def test_failure_injection_into_every_restartable_node(self):
        ...
        for index, key in enumerate([node["id"] for node in self.data["nodes"]]):
            with self.subTest(failed_node=key):
                ...
                if key not in {"extract_orders", "extract_customers"}:
                    self.assertEqual(
                        second["nodes"]["extract_orders"]["status"], Status.SKIPPED_ALREADY_SATISFIED
                    )
                if key not in main:
                    self.data["runtime"]["fault_injection"].clear()
...
class ProcessExecutionTests(DeploymentTest):
    def test_real_child_process_happy_path(self):
        self.assert_report(self.run_dag(real_processes=True))

    def test_timeout_terminates_worker_before_recovery(self):
        self.node("extract_orders")["timeout_seconds"] = 0.5
        self.node("extract_orders")["retry"]["max_retries"] = 0
        self.fault("extract_orders", "timeout", delay_seconds=20)
        started = time.monotonic()
        first = self.run_dag(real_processes=True)
        self.assertLess(time.monotonic() - started, 20)
        self.assertEqual(first["nodes"]["extract_orders"]["status"], Status.TIMED_OUT)
        self.assertFalse(first["nodes"]["extract_orders"]["execution_uncertain"])
        self.assertEqual(first["nodes"]["notify_timeout"]["status"], Status.SUCCEEDED)
        self.data["runtime"]["fault_injection"].clear()
        self.node("extract_orders")["timeout_seconds"] = 30
        second = self.run_dag("resume", first["run_id"], real_processes=True)
        self.assert_report(second)
        self.assertEqual(second["nodes"]["extract_customers"]["status"], Status.SKIPPED_ALREADY_SATISFIED)

    def test_timeout_retry_has_no_overlapping_attempts(self):
        self.node("extract_orders")["timeout_seconds"] = 3
        self.fault("extract_orders", "timeout", delay_seconds=20)
        ...
        attempts = [
            ...
            if event["event_type"] == "ATTEMPT_FINISHED" and event["node_id"] == "extract_orders"
        ]
    ...
    def test_crash_before_checkpoint_retries_safely(self):
        self.fault("extract_orders", "crash")
        result = self.run_dag(real_processes=True)
        self.assert_report(result)
        self.assertEqual(result["nodes"]["extract_orders"]["attempt"], 2)
